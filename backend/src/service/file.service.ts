...
export class FileService {
    ...
    async updateFile(id: string, updatedData: { fileName?: string, folderId?: string }) {
        const file = await FileRepository.findById(id);
        if (!file) {
        ...
        if (updatedData) {
            partialFile.storagePath = file.storagePath.split("/").slice(0, -1).join("/") + "/" + updatedData.fileName;
        }
